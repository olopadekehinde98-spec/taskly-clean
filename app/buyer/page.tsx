...
function statusColor(status: string) {
  ...
  if (status === 'active' || status === 'in_progress') return 'bg-[#edfbf2] text-[#28a84e]'
      ...
      <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
          <div className="mb-6 flex items-center justify-between">
            ...
            <Link href="/buyer/orders" className="text-sm text-[#3ecf68] hover:underline">View all</Link>
          </div>

          {!recentOrders || recentOrders.length === 0 ? (
            <div className="rounded-2xl bg-slate-50 p-8 text-center">
              <p className="text-slate-500 text-sm">No orders yet.</p>
              <Link href="/services" className="mt-3 inline-block rounded-xl bg-[#3ecf68] px-4 py-2 text-sm text-white">Browse Services</Link>
            </div>
          ) : (
        ...
        <section className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="space-y-3">
            <Link href="/services" className="flex items-center gap-3 rounded-2xl bg-[#edfbf2] p-4 text-sm font-medium text-[#28a84e] hover:bg-[#d4f5e4] transition-colors">
