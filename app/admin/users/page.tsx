                    ...
                    <div className="flex gap-2 flex-wrap">
                      {(!u.account_status || u.account_status === 'active') && !u.is_admin && (
                        <>
                          <form action={updateUserStatus}>
                            ...
                            <button className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-1 text-xs text-amber-700 hover:bg-amber-100">Restrict</button>
                          </form>
                          <form action={updateUserStatus}>
                            <input type="hidden" name="user_id" value={u.id} />
                            <input type="hidden" name="action" value="suspend" />
                            <input type="hidden" name="reason" value="Admin suspension" />
                            <button className="rounded-lg border border-orange-300 bg-orange-50 px-3 py-1 text-xs text-orange-700 hover:bg-orange-100">Suspend</button>
                          </form>
                          <form action={updateUserStatus}>
                            <input type="hidden" name="user_id" value={u.id} />
                            <input type="hidden" name="action" value="ban" />
                            <input type="hidden" name="reason" value="Admin ban" />
                            <button className="rounded-lg border border-red-300 bg-red-50 px-3 py-1 text-xs text-red-700 hover:bg-red-100">Ban</button>
                          </form>
                        </>
                      )}
                      {u.account_status && u.account_status !== 'active' && (
                        <form action={updateUserStatus}>
                          ...
                          <button className="rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-1 text-xs text-emerald-700 hover:bg-emerald-100">Restore</button>
                        </form>
                      )}
                    </div>
                  </td>
